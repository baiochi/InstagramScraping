...
# Post List
with open(folder + '\\' + profile_name + '_' + nameof(post_list) + '.pkl','wb') as pickle_file:
        pickle.dump(post_list, pickle_file)
# Post Likes
with open(folder + '\\' + profile_name + '_' + nameof(post_likes) + '.pkl','wb') as pickle_file:
        pickle.dump(post_likes, pickle_file)
# Followers profile
with open(folder + '\\' + profile_name + '_' + nameof(followers_profile) + '.pkl','wb') as pickle_file:
        pickle.dump(followers_profile, pickle_file)
# Data Frame
with open(folder + '\\' + profile_name + '_' + nameof(df) + '.pkl','wb') as pickle_file:
        pickle.dump(df, pickle_file)

final_time = datetime.now() - init_time

print('Scrap complete!')
print(f'Timelapse: {final_time.seconds//3600} : {(final_time.seconds//60)%60} : {final_time.seconds}')
